...
void ADMCTestAdSecurity::add_right() {
    SecurityRight right{SEC_ADS_CREATE_CHILD, QByteArray(), QByteArray()};
...
void ADMCTestAdSecurity::handle_generic_read_and_write_sharing_bit() {
    ...
    for (const uint32_t &mask : opposite_map.keys()) {
        ...
        SecurityRight right{mask, QByteArray(), QByteArray()};
        SecurityRight right_opposite{mask, QByteArray(), QByteArray()};
        security_descriptor_add_right(sd, ad.adconfig(), class_list, test_trustee, right, true);
...
void ADMCTestAdSecurity::add_to_unset_opposite() {
    ...
    SecurityRight right{(uint32_t)access_mask, object_type, QByteArray()};
...
void ADMCTestAdSecurity::remove_to_set_subordinates() {
    ...
    SecurityRight right{(uint32_t)superior_mask, QByteArray(), QByteArray()};
...
void ADMCTestAdSecurity::remove_to_unset_superior() {
    ...
    // Remove subordinate
    SecurityRight right{(uint32_t)subordinate_mask, subordinate_right_type, QByteArray()};
...
void ADMCTestAdSecurity::add_to_unset_opposite_superior() {
    ...
    SecurityRight right_generic{SEC_ADS_GENERIC_ALL, QByteArray(), QByteArray()};
    SecurityRight right_create_child{SEC_ADS_CREATE_CHILD, QByteArray(), QByteArray()};
...
void ADMCTestAdSecurity::check_state(const QByteArray &trustee, const uint32_t access_mask, const QByteArray &object_type, const TestAdSecurityType type) const {
    SecurityRight right_generic{access_mask, object_type, QByteArray()};
