...
%changelog
* Fri Sep 20 2024 Semyon Knyazev <samael@altlinux.org> 0.17.1-alt1
- Delegation permissions are added to "Security" tab from object
  properties. These permissions include common tasks (for organizational
  unit and container classes only), create/delete permissions for
  possible child objects and read/write permissions for object's attributes
  (properties).
- Common, extended and delegation permissions are splitted and placed
  in corresponding tabs.
