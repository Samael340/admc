...
void PermissionsWidget::on_item_changed(QStandardItem *item) {
    ...
    SecurityRight right{access_mask, object_type, QByteArray()};

    if (checked) {
        security_descriptor_add_right(sd, g_adconfig, target_class_list, trustee, right, allow, flags);
    } else {
        security_descriptor_remove_right(sd, g_adconfig, target_class_list, trustee, right, allow, flags);
    }
