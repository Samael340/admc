...
bool AdInterface::gpo_check_perms(const QString &gpo, bool *ok) {
    ...
    const QString gpt_sd = [&]() {
        ...
        while (true) {
            const int getxattr_result = d->smb_context.smbcGetxattr(smb_path_cstr, "system.nt_sec_desc.*", buffer, buffer_size);
...
void AdInterface::update_dc() {
    ...
    ldap_free();
    if (!d->smb_context.is_valid()) {
        d->smb_context = SMBContext();
    }

    d->is_connected = ldap_init() && d->smb_context.is_valid();
}
